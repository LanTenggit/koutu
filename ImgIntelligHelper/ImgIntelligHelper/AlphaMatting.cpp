...
//载入图像
void AlphaMatting::loadImage(char* filename)
{
...
//载入第三方图像
void AlphaMatting::loadTrimap(char* filename)
{
...
double AlphaMatting::comalpha(Scalar c, Scalar f, Scalar b)
{
...
double AlphaMatting::mP(int i, int j, Scalar f, Scalar b)
{
...
double AlphaMatting::nP(int i, int j, Scalar f, Scalar b)
{
	int i1 = max(0, i - 1);
...
double AlphaMatting::eP(int i1, int j1, int i2, int j2)
{
...
double AlphaMatting::pfP(Point p, vector<Point>& f, vector<Point>& b)
{
	double fmin = 1e10;
...
double AlphaMatting::aP(int i, int j, double pf, Scalar f, Scalar b)
{
	int bc = data[i * step + j * channels];
	int gc = data[i * step + j * channels + 1];
	int rc = data[i * step + j * channels + 2];
...
double AlphaMatting::dP(Point s, Point d)
{
	return sqrt(double((s.x - d.x) * (s.x - d.x) + (s.y - d.y) * (s.y - d.y)));
}

double AlphaMatting::gP(Point p, Point fp, Point bp, double pf)
{
	int bc, gc, rc;
...
double AlphaMatting::gP(Point p, Point fp, Point bp, double dpf, double pf)
{
	int bc, gc, rc;
	bc = data[fp.x * step + fp.y * channels];
	gc = data[fp.x * step + fp.y * channels + 1];
...
double AlphaMatting::sigma2(Point p)
{
	int xi = p.x;
	int yj = p.y;
	int bc, gc, rc;
...
double AlphaMatting::distanceColor2(Scalar cs1, Scalar cs2)
{
...
void AlphaMatting::sample(Point p, std::vector<Point> &f, std::vector<Point> &b)
{
	int i = p.x;
	int j = p.y;
...
void AlphaMatting::Sample(std::vector<vector<Point> > &F, std::vector<vector<Point> > &B)
{
	int   a, b, i;
	int   x, y, p, q;
	int   w, h, gray;
...
void AlphaMatting::refineSample()
{
...
//存储图像
void AlphaMatting::save(char* filename)
{

	imwrite(filename, matte);
}

void AlphaMatting::getMatte()
{
	int h = matte.rows;
	int w = matte.cols;
	int s = matte.step1();
...
uchar* AlphaMatting::GetMatteData()
{
    int h = matte.rows;
	int w = matte.cols;
	int s = matte.step1();
	
	cout << "Matte.Step1() Value: " << s << endl;
...
int AlphaMatting::GetAlphaLength() {
	int h = matte.rows;
	int w = matte.cols;
	return h * w;
}

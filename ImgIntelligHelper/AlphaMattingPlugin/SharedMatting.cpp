...
void SharedMatting::loadImage(char* filename)
{
	...
	result.create(Size(width, height), CV_8UC4);
}
//载入第三方图像
...
void SharedMatting::save(char* filename)
{
	...
	int resChannel, resStep;
	resChannel = result.channels();
	resStep = result.step1();
	uchar* d = (uchar*)result.data;
	for (int i = 0; i < height; ++i)
	{
		for (int j = 0; j < width; ++j)
		{
			d[i * resStep + j * resChannel] = data[i * step + j * channels];
			d[i * resStep + j * resChannel + 1] = data[i * step + j * channels + 1];
			d[i * resStep + j * resChannel + 2] = data[i * step + j * channels + 2];
			d[i * resStep + j * resChannel + 3] = alpha[i][j];
		}
	}
